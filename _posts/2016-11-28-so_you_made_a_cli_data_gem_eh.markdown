...
"Why would you initialize a _whole new_ `Scraper` object to make an article out of? You literally have the link right there in front of you! Go scrape that directly!" - You right now
...
"Okay, well why don't you just pull the suffix of the link it points to, you know, the part of the article URL where it's the literal term you'd want to use to get that (and only that) webpage, and use that to build the `Scraper` object, instead of the potentially ambiguous hash key string?" - You right now
